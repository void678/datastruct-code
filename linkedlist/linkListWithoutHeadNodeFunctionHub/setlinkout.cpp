#include<iostream>
#include"linkListWithoutHeadNodeFunctionHub.h"
using namespace std;
...
void UnionList(LinkList LC, LinkList LD, LinkList &LE) {
    ...
    for (int i = 1; i <= Ld_len; ++i) {
        ...
        //cout<<"e="<<e<<endl;
        if (LocateElem(LE, e)) {
            //cout<<"++len="<<Le_len<<"  e="<<e<<" loc="<<LocateElem(LE,e)<<endl;
            continue;
        } else {
            //cout<<"++len="<<Le_len<<"  e="<<e<<endl;
...
int main() {
    ...
    printf("num = ");
    cin>>la_len;
    LinkedListCreateTailInsert(LA,la_len);

    printf("********************Input LB********************\n");
    printf("num = ");
    cin>>lb_len;
    LinkedListCreateTailInsert(LB,lb_len);
